...
app.get("/api/lobby/:lobbyId", authenticateToken, async (req, res) => { // An Array containing all the messages from the lobby
    ...
    try {
        const currentLobby = await Lobby.findOne({_id: lobbyId}); // Search the lobby in the database
        currentLobby.users.forEach(user => { // Check if the user is in the lobby
            if(user !== userName){
    ...
    try {
        ...
        currentLobby.messages.forEach(message => { // Check if the message is in the lobby
            if(message._id === messageId){
                return res.json({error: false, message: "Here is the message", lobby:currentLobby.name, msg: message});
            }
        });
...
app.patch("/api/lobby/:lobbyId/:messageId", authenticateToken, async (req, res) => { // Both the owner and admin can edit the messages
    const {content, owner, admin} = req.body;
    const lobbyId = req.params.lobbyId;
    const messageId = req.params.messageId;
    ...
    try {
        ...
        if(owner !== currentMessage.owner || admin !== currentLobby.admin){ // User is not the owner or the admin
            return res.status(400).json({message: "Only the owner or the admin can edit the message"});
        }
...
app.get("/api/:lobbyId/users", authenticateToken, async (req, res)=>{ // All user from the lobby. Admin only?
    const lobbyId = req.params.lobbyId;
...
app.delete("/api/messages/:messageId", authenticateToken, async (req, res) => { // Delete a message, Admin Only
    ...
    try {
        ...
        const lobby = await Lobby.findOne({name: message.lobbyName}); // Search the lobby in the database
